...
public class reservationMat {
    ...
    int idAdh;
    String getLot= "SELECT * FROM Lot WHERE marque = ? AND modele= ? AND anne_achat= ?;";
    String updateLot = "UPDATE lot set nb_pieces_lot = nb_pieces_lot - ? WHERE marque = ?, modele= ?, anne_achat= ?";
    ...
    public reservationMat(Connection connection, Map<Lot,Integer> listLotReserve, int idAdh, Date  dateEmprunt, Date dateRetour){
    ...
    public void makeReservation(){
        ...
        try
        {
            ...
            for (Lot lot : listLotReserve.keySet()){
                ...
                if (result.getInt("nb_pieces_lot")<=qtt){
